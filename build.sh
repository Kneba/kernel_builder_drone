...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	then
		msg "|| Cloning GCC  ||"
		git clone --depth=1 https://github.com/Baka-Project/BakaGCC -b gcc-arm64-nonelf $KERNEL_DIR/gcc64
		git clone --depth=1 https://github.com/Baka-Project/BakaGCC -b gcc-arm-nonelf $KERNEL_DIR/gcc32
...
exports() {
	...
	then
		KBUILD_COMPILER_STRING=$("$GCC64_DIR"/bin/aarch64-nonelf-gcc --version | head -n 1)
...
build_kernel() {
	...
	then
		make -j"$PROCS" O=out \
				...
				CROSS_COMPILE=aarch64-nonelf- \
				AR=aarch64-nonelf-ar \
				OBJDUMP=aarch64-nonelf-objdump \
				STRIP=aarch64-nonelf-strip  \
				LD="ld.lld"
